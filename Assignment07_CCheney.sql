...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_CCheney')
	 Begin 
	  Alter Database [Assignment07DB_CCheney] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_CCheney;
	 End
	Create Database Assignment07DB_CCheney;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_CCheney;
...
JOIN vProducts p ON c.CategoryID = p.CategoryID
JOIN vInventories i ON p.ProductID = i.ProductID
GROUP BY c.CategoryName, DATENAME(month, i.InventoryDate), DATENAME(year, i.InventoryDate);
go
...
SELECT 
    ...
    InventoryDate,
    Count,
    ISNULL(LAG(Count) OVER (PARTITION BY ProductName ORDER BY InventoryDate), 0) AS PreviousMonthCount
